...
class Client:
    ...
    def init(self):
      ...
      if not len(result) == 1:
          cursor.execute(f"CREATE TABLE `{self.database}`.`hosts` (`ID` INT NOT NULL AUTO_INCREMENT , `HOST_NAME` VARCHAR(255) NULL , `CPU_MAX` DECIMAL(10,3) NULL , `CPU_NOW` DECIMAL(10,3) NULL , `MEMORY_MAX` DECIMAL(10,3) NULL , `MEMORY_NOW` DECIMAL(10,3) NULL , `TX_MAX` BIGINT NULL , `TX_NOW` BIGINT NULL , `RX_MAX` BIGINT NULL , `RX_NOW` BIGINT NULL , `CPU_NAME` VARCHAR(255) NULL , `OS_NAME` VARCHAR(255) NULL , `OS_TIME` TIMESTAMP NULL , `OS_TIMEZONE` VARCHAR(255) NULL , `UPTIME` BIGINT NULL , `STORAGE_MAX` DECIMAL(10,2) NULL , `CPU_CORES` INT NULL , `STORAGE_USED` DECIMAL(10,2) NULL , PRIMARY KEY (`ID`)) ENGINE = InnoDB;")
          self.client.commit()
